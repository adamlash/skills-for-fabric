...
LH_ID="<lakehouse-guid>"           # from grounding (sourceTableProperties.itemId)
SQL_HOST="<sql-endpoint-host>"     # from `get item` on LH (NOT in ontology grounding)

az rest --resource https://api.fabric.microsoft.com \
  --uri "https://${SQL_HOST}/v1.0/myorg/databases('${LH_ID}')/Tables('panels')?\$filter=id eq 'panel7'" \
  --output table
```
...
4. **Read metadata from grounding, not from the API.** Item GUIDs, cluster URIs, SQL endpoint hosts, source-column names, linking-table names, and `databaseName` are all in the in-memory grounding JSON the moment you decoded the ontology. A walk that issues `list items` / `get eventhouse` / `take 2` mid-flight is a walk that hasn't done its [Pre-Walk Checklist](#pre-walk-checklist-do-this-once-per-session-then-never-again).
...
- **Schema gaps surfaced during walk** (missing relationship, mis-typed binding) → escalate to `ontology-authoring-cli`, do not patch in-flight.
