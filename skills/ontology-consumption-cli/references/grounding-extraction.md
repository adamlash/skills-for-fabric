...
See [ONTOLOGY-AUTHORING-CORE.md § Definition Tree](../../../common/ONTOLOGY-AUTHORING-CORE.md#definition-tree) for the authoritative file-and-folder shape of the parts you will decode — reading it is the best way to understand every field the summary exposes.
...
# 1. Kick off the LRO, capture the Location header
...
if [ -z "$LOC" ]; then
  echo "Failed to capture Location header from getDefinition" >&2
  exit 1
fi
...
while :; do
  ...
  case "$STATE" in
    Succeeded) break ;;
    Failed|Canceled) echo "LRO ended in state: $STATE" >&2; exit 1 ;;
    *) sleep 2 ;;
  esac
done
...
# 1. Kick off LRO; az rest emits the Location header on the debug channel
...
$loc = ($verbose -split "`n" | Where-Object { $_ -match 'Location:\s*(\S+)' } | ForEach-Object { $matches[1] } | Select-Object -First 1)
if (-not $loc) { throw "No Location header captured from getDefinition" }

# 2. Poll
while ($true) {
  $state = (az rest --method GET --url $loc --resource "https://api.fabric.microsoft.com" | ConvertFrom-Json).status
  switch ($state) {
    "Succeeded"         { break }
    { $_ -in @("Failed","Canceled") } { throw "LRO ended: $state" }
...
# 3. Retrieve result
$def = az rest --method GET --url "$loc/result" --resource "https://api.fabric.microsoft.com" | ConvertFrom-Json
...
After decoding, `${OUT_DIR}/` mirrors the authoring tree documented in [ONTOLOGY-AUTHORING-CORE.md § Definition Tree](../../../common/ONTOLOGY-AUTHORING-CORE.md#definition-tree):
      ...
      "contextualizations": [
        {
          ...
          "sourceKeyBindings": [ { "columnName": "<source-side key column>" } ],
          "targetKeyBindings": [ { "columnName": "<target-side key column>" } ]
        }
      ]
    }
  ]
...
| `relationshipTypes[].contextualizations[]` | `RelationshipTypes/{id}/Contextualizations/{guid}.json` | Linking table + key column **arrays** (`sourceKeyBindings[]` / `targetKeyBindings[]`, composite-key safe) for realising the relationship |
