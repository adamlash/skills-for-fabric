...
See [ONTOLOGY-AUTHORING-CORE.md § EntityType file](../../../common/ONTOLOGY-AUTHORING-CORE.md#entitytype-file--entitytypesiddefinitionjson) for the complete schema and the source-column → `valueType` mapping table.
...
Binding constraint checklist (see [ONTOLOGY-AUTHORING-CORE.md § Binding rules](../../../common/ONTOLOGY-AUTHORING-CORE.md#binding-rules) for the full list):
...
Both `createItem` and `updateDefinition` return **long-running operations**. `getDefinition` is also LRO-capable — it may return the envelope directly (`200 OK`) **or** a `202 Accepted` with a `Location` header pointing at the operation; in the 202 case, poll until `Succeeded` and then `GET {Location}/result` to retrieve the envelope. See [COMMON-CLI.md § Long-Running Operations (LRO) Pattern](../../../common/COMMON-CLI.md#long-running-operations-lro-pattern) and the fetch-mutate-send script in [definition-script-templates.md](definition-script-templates.md), which handles both cases.

**When renaming the ontology**, add the `updateMetadata=true` query string to `updateDefinition` and include an updated `.platform` part. See [ONTOLOGY-AUTHORING-CORE.md § Item-management contract](../../../common/ONTOLOGY-AUTHORING-CORE.md#item-management-contract).
