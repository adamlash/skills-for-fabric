...
|---|---|---|---|
| Create ontology item | `POST` | `/v1/workspaces/{ws}/items` | Body: `{"displayName","type":"Ontology","definition":{"parts":[...]}}`. Long-running; response returns `202` with `Location` header pointing at the operation. Creation can also be done with only `.platform` + empty `definition.json`, then populate via `updateDefinition`. |
| Get ontology definition | `POST` | `/v1/workspaces/{ws}/items/{id}/getDefinition` | Long-running. `200` returns the envelope directly. `202` returns `Location` header; poll until `Succeeded`, then `GET {Location}/result` to retrieve the envelope. |
